resource "aws_security_group_rule" "rule_egress" {
  ...
  source_security_group_id = var.to_security_group_id
}

resource "aws_security_group_rule" "rule_ingress" {
  description       = "Allow ${var.from_name} to ${var.to_name} connection (ingress rule)"
  type              = "ingress"
  from_port         = var.port
  to_port           = var.port
  protocol          = var.protocol
  security_group_id = var.to_security_group_id
