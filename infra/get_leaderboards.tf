...
module "get_leaderboards_lambda" {
  source  = "./modules/lambda"
  context = module.null_label.context

  name            = "get-leaderboards-lambda"
  handler         = "handler.handler"
  source_dir      = "${path.root}/../backend/dist/api_gateway/get_leaderboards"
  build_path      = "${path.root}/../backend/build/api_gateway/get_leaderboards/get_leaderboards.zip"
  runtime         = "nodejs20.x"
  memory          = 256
  time_limit      = 60
  deployment_type = "zip"
...
module "sg_leaderboard_lambda_to_rds" {
  source                 = "./modules/sg/each_other"
  from_name              = module.get_leaderboards_lambda.name
  to_name                = "RDS"
  from_security_group_id = module.get_leaderboards_lambda.security_group_ids[0]
