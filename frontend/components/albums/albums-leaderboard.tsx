import { LeaderboardSummary, AlbumLeaderboardEntry } from "@/types/leaderboard";
import { Trophy } from "lucide-react";

interface AlbumsLeaderboardProps {
  leaderboard: AlbumLeaderboardEntry[];
}

export const AlbumsLeaderboard = ({ leaderboard }: AlbumsLeaderboardProps) => {
  if (!leaderboard || leaderboard.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">
        No albums in the leaderboard yet
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              Top albums by total plays in the last 30 days
            </p>
          </div>
        </div>
      </div>
