...
import { ArtistsLeaderboard } from "@/components/artists/artists-leaderboard";

export default function Artists() {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [summary, setSummary] = useState<LeaderboardSummaryType | null>(null);
  const [artistLeaderboard, setArtistLeaderboard] = useState<
    ArtistLeaderboardEntry[] | null
  >(null);
    ...
    const fetchData = async () => {
      try {
        ...
        if (
          ...
          setArtistLeaderboard(null);
          setSummary(null);
          return;
        }

        setArtistLeaderboard(
          response.data.leaderboard as ArtistLeaderboardEntry[]
        );
        ...
        <Container className="-translate-y-10 pt-20 relative overflow-visible">
          ...
          <div className="grid grid-cols-3 gap-x-10 relative min-h-screen">
            <div className="col-span-2">
              <ArtistsLeaderboard leaderboard={artistLeaderboard!} />
            </div>
